    ...
    try {
        ...
        isExists.forEach(async (isExists) => {
            ...
            if (expireDate <= currentDate) {
                const couponData = await Coupon.findOneAndUpdate({ name: isExists.name }, { $set: { validity: "expired" } })
            }
        })
...
const makeOrderChanges = async (req, res) => {
    try {
        const orderData = await Order.findByIdAndUpdate({ _id: req.query.id }, { $set: { orderStatus: req.body.orderStatus } })
        if (orderData) {
...
const restoreTrash = async (req, res) => {
    try {
        ...
        if (remove) {
            const trashData = await Trash.findByIdAndDelete({ _id: req.query.id })

        }
    } catch (error) {
        res.render('trash', { message: "Something went wrong" })
        console.log(error.message);
    }
}
module.exports = {
